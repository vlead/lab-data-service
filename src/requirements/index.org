#+TITLE: Lab Data Service
#+AUTHOR: VLEAD
#+DATE: [2016-11-30 Wed]
#+PROPERTY: results output
#+PROPERTY: exports code
#+SETUPFILE: ../org-templates/level-1.org
#+options: ^:nil
#+LATEX: Literal LaTeX code for export

* Introduction 
  This document describes all the requirements of =lab-data-service=.


* Requirements:

** Get lab's meta data:
   - Get all the labs meta data which includes lab id, experiment id,
     institutes, disciplines and developers...,etc details.
   :PROPERTIES:
   :CUSTOM_ID: lab_meta_data
   :END: 

** Construct lab spec with lab's meta data in json format :
   - Lab spec should be filled up with consistant naming convention of
     lab id, experiment id, institutes, disciplines and developers
     details..,etc in =json= format
:PROPERTIES:
:CUSTOM_ID: construct_lab_spec
:END: 

** Version control lab spec file :
   - All the lab spec files has to be version controled into single git repository.
:PROPERTIES:
:CUSTOM_ID: version_lab_specs
:END: 
   
** Parse lab spec files checked into git repo :
   - A =web-hook= from Github repository will invoke the
     =lab-data-service= when changes made to lab specs.
     
   - =lab-data-service= will parse the lab spec files and store the
     data into the database.
:PROPERTIES:
:CUSTOM_ID: parse_lab_specs
:END: 
** Lab-data-service should answer for all lab related queries
   - This service should answer queries like get me all experiments of a
     given lab and get me devoleper details of a given lab..,etc.
   :PROPERTIES:
   :CUSTOM_ID: lab_queries
   :END: 

** All lab spec files in git repo are read only  :
   - Only admin or group of authorized users of git repository has
     access to modify the lab spec files.
:PROPERTIES:
:CUSTOM_ID: lab_spec_read_only
:END: 

* Design diagram
#+BEGIN_HTML
<img width="600px" src="https://docs.google.com/drawings/d/e/2PACX-1vQ-WiohgmhaseluocCU0TBOoIUSWctv6raQ03gPO_-jhRkgtQbxH2DX0924ux3O7Hfz3zwKGJnt7r1Z/pub?w=960&h=720">
<p align="center"> lab-data-service </p>
#+END_HTML

  To edit the feedback relations image [[https://docs.google.com/drawings/d/1TAaHpf0UiCLvLRP_XfBBQki7pU624oWXRduxyz6e1NA/edit?ts=58415ec0][click here]]
