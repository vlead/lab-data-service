...
* Developer
=Developer= is a class with an attribute =dev_name= of type alphabetic string and
=email_id= of alphabetic string.
...
args = {"__tablename__": "dev",
        ...
        "dev_name": db.Column(db.String(255), unique=False, 
                                       nullable=True)
        }

Dev = ClassPersistenceTemplate.mk_persistent(Dev, ['email_id', 'dev_name'],\
                                                 **args)
...
class TestPersistentDeveloper(TestCase):
    ...
    def test_create_lab(self):
        ...
        self.assertEqual(dev.get("dev_name"), dev_name)
        self.assertEqual(dev.get("email_id"), email_id)
