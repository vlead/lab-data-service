...
* Assets
  ...
  =Assets= is a class with attributes =path= and =asset_type= of type =is_str=.
#+NAME: class_Assets
#+begin_src python
args = {"__tablename__": "assets",
        "id": db.Column(db.Integer, primary_key=True),
...
Assets = ClassPersistenceTemplate.mk_persistent(Assets, ['path'], **args)
...
#+BEGIN_SRC python
class TestPersistenceAssets(TestCase):
    TESTING = True
