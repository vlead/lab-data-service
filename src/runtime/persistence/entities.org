...
lab_hostinginfos = db.Table('lab_hostinginfos',
                 db.Column('labid', db.Integer, 
                       db.ForeignKey('lab.id')),
		 db.Column('hostinginfoid', db.Integer,
                       db.ForeignKey('hostinginfo.id')))
...
args = {"__tablename__": "lab",
        ...
        "integrationstatus_id": db.Column(db.Integer, db.ForeignKey('integrationstatus.id'),
                                      unique=False, nullable=False),
        ...
        "developers": db.relationship('Developer',
                    ...
                    backref='labs'),
        "hostinginfos": db.relationship('HostingInfo',
                    secondary=lab_hostinginfos, 
                    backref='labs')
       }
...
class TestPersistentLab(TestCase):
    ...
    def test_create_lab(self):
        ...
        lab2 = Lab(lab_name=lab_name2, lab_id=lab_id2, overview=overview2,
                       ...
                       developers=[], hostinginfos=[host], assets=[asset], 
                       integrationstatus=intstatus)
        lab2.save()
        ...
        self.assertEqual(lab1.get("integrationstatus").get("integration_level"), 
                             intstatus.get("integration_level"))

        self.assertEqual(lab1.get("developers")[0].get("email").get("email"), 
                             email_id.get("email"))

        self.assertEqual(lab1.get("hostinginfos")[0].get("hosting_status"), 
        ...
        self.assertEqual(lab2.get("discipline").get("dis_name"), 
                             discipline.get("dis_name"))
        self.assertEqual(lab2.get("hostinginfos")[0].get("hosting_status"), 
                             host.get("hosting_status"))
        self.assertEqual(lab2.get("experiments"), [])

        self.assertEqual(lab2.get("assets")[0].get("asset_type"), 
                             asset.get("asset_type"))
        self.assertEqual(lab2.get("integrationstatus").get("integration_level"), 
                             intstatus.get("integration_level"))
...
experiments_hostinginfos = db.Table('experiments_hostinginfos',
                 db.Column('experimentid', db.Integer, 
                       db.ForeignKey('experiment.id')),
		 db.Column('hostinginfoid', db.Integer,
                       db.ForeignKey('hostinginfo.id')))
...
args = {"__tablename__": "experiment",
        ...
        "intstatus_id": db.Column(db.Integer, db.ForeignKey('integrationstatus.id'),
                                      unique=False, nullable=False),
        "assets": db.relationship('Asset',
        ...
        "developers": db.relationship('Developer',
                    ...
                    backref='experiments'),
        "hostinginfos": db.relationship('HostingInfo',
                    secondary=experiments_hostinginfos, 
                    backref='experiments')
        }
...
class TestPersistenceExperiment(TestCase):
    ...
    def test_create_experiment(self):
        ...
        exp = Experiment(exp_name=exp_name, exp_id=exp_id, overview=overview,
                          integrationstatus=intstatus, assets=[asset], 
                         sections=[], developers=[dev], hostinginfos=[host],  
                         institute=inst, discipline=discipline)
        exp.save()
        ...
        self.assertEqual(experiment.get("institute").get("inst_name"), 
                             inst.get("inst_name"))
        self.assertEqual(experiment.get("discipline").get("dis_name"), 
                             discipline.get("dis_name"))
        ...
        self.assertEqual(experiment.get("developers")[0].get("email").get("email"), 
                             email_id.get("email"))
        self.assertEqual(experiment.get("hostinginfos")[0].get("hosting_status"), 
                             host.get("hosting_status"))
...
args = {"__tablename__": "institute",
        ...
        "experiments": db.relationship('Experiment', backref=db.backref('institute')),
        "developers": db.relationship('Developer', backref=db.backref('institute'))
       }
...
args = {"__tablename__": "discipline",
        ...
        "experiments": db.relationship('Experiment', backref=db.backref('discipline')),
        "developers": db.relationship('Developer', backref=db.backref('discipline'))
        }
...
#+NAME: class_exp_association_Table
#+BEGIN_SRC python
developers_institutes = db.Table('developers_institutes',
		      db.Column('developer_id', db.Integer, 
                            db.ForeignKey('developer.id')),
		      db.Column('institute_id', db.Integer,
                            db.ForeignKey('institute.id')))

developers_disciplines = db.Table('developers_disciplines',
		      db.Column('developer_id', db.Integer, 
                            db.ForeignKey('developer.id')),
...
class TestPersistentDeveloper(TestCase):
    ...
    def test_create_dev(self):
        ...
        self.assertEqual(dev.get("institute").get("inst_name"), 
                             inst.get("inst_name"))
        self.assertEqual(dev.get("discipline").get("dis_name"), 
                             discipline.get("dis_name"))
...
#        "labs": db.relationship('Lab', backref=db.backref('hostinginfo')),
#        "experiments": db.relationship('Experiment', backref=db.backref('hostinginfo'))
       }
...
#        "labs": db.relationship('Lab', backref=db.backref('asset'))
       }
...
#+begin_src python
args = {"__tablename__": "integrationstatus",
        "id": db.Column(db.Integer, primary_key=True),
        "integration_level": db.Column(db.Integer, unique=True, nullable=False),
        "labs": db.relationship('Lab', backref=db.backref('integrationstatus')), 
        "experiments": db.relationship('Experiment', backref=db.backref('integrationstatus')), 
...
class TestPersistenceIntegrationStatus(TestCase):
    ...
    def test_create_integrationstatus(self):
        print "test_persistance_integrationstatus"
        integration_level = 4
        intstatus = IntegrationStatus(integration_level=integration_level)
        intstatus.save()
        integrationstatus = IntegrationStatus.get_by_id(1)
        integrationstatus1 = IntegrationStatus.get_by_id(2)
        self.assertEqual(integrationstatus.get("integration_level"), integration_level)
        self.assertEqual(integrationstatus1,None)
