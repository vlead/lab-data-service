...
Assets = ClassTemplate.mk_class("Assets")
...
* Assets
   =Assets= is an entity that has a field =type= and =path= which are string
#+NAME: class_Assets
#+begin_src python
Assets.add_attributes(path=is_str,
                    asset_type=is_str)

Assets.__eq__ = lambda self, other: \
                  isinstance(other, self.__class__) and \
...
*** Test Assets
#+NAME: class_TestAssets
#+BEGIN_SRC python
class TestAssets(TestCase):
    TESTING = True
    def test_object_assets(self):
        print "test_object_assets"
        path = "vlabs.ac.in/static/images/logo.png"
        asset_type = "Image"
        assets = Assets(path=path, asset_type=asset_type)
                                              
        self.assertEqual(assets.get("path"), path)
        self.assertEqual(assets.get("asset_type"), asset_type)
#+end_src


* HostingInfo
   =HostingInfo= is an entity that has a field =hosting_status= and =hosted_url= which are string
...
HostingInfo.__eq__ = lambda self, other: \
                  ...
                  self.get("hosted_url") == other.get("hosted_url")
#+end_src
