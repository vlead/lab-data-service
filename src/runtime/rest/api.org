...
** Get Labs, Get Lab, Get Labs by institute id, institute name, lab_name, discipline id, discipline name and Get labs by passing lab_name as keyword
*** API Designs 
**** Get all labs
  :PROPERTIES:
...
def get_lab():
    if request.method == 'GET':
        ...
        elif 'hosted_on' in request.args:
            hosted_on = request.args['hosted_on']
            try:
                APP_URL = "http://localhost"
                current_app.logger.debug("APP_URL: %s" % APP_URL)
		r = requests.get(APP_URL + "/labs")
                labs = r.json()
                lab_list = []
                for lab in labs:
                    if len(lab['hosting_info'])!= 0:
                        for i in lab['hosting_info']:
                            if i['hosted_on'] == hosted_on:
				lab_list.append(lab)
                current_app.logger.debug("completed operation get_labs_by_hosted_on")
                return jsonify_list(lab_list)
