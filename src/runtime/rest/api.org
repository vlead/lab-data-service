...
def get_and_add_institute():
    if request.method == 'GET':
        if 'inst_id' in request.args:
            ...
            try:
                current_app.logger.debug("execute get_institute_by_instituteid")
                institute = SystemInterface.get_institute(inst_id)
                current_app.logger.debug("executed get_institute_by_instituteid")
...
#+BEGIN_SRC python
@api.route('/institutes', methods=['GET', 'PUT', 'DELETE'])
