...
#+END_EXAMPLE
  ...
  - Example:
#+BEGIN_EXAMPLE
    http://localhost:5000/map?expid=exp123
#+END_EXAMPLE
...
def generate_experiment_resource_map():
    if request.method == 'GET':
        if 'expid' in request.args:
            exp_id  = request.args['expid']
            try:
                current_app.logger.debug("execute generate_exp_resource_map")
                exp = SystemInterface.get_experiment(exp_id)
                experiment_resource_maps = {}
                sections = exp['sections']
                for section in sections:
                    experiment_resource_maps[section['name']] = section['name'].lower() + '-' + exp_id
                current_app.logger.debug("executed generate_exp_resource_map")
                data = {"experiment_resource_maps": experiment_resource_maps}
                return jsonify(data)
...
*** Test Cases
**** TestResourceById
 #+NAME: test_get_resource_by_id
#+BEGIN_SRC python
class TestResourceById(TestCase):
    TESTING = True
    ...
    def test_get_resource_by_id(self):
        ...
        response = self.client.get("/map?exp_id=e99847", data=json.dumps(payload),
                                    headers=headers)

        self.assertEqual(response.status_code, 200)

#+END_SRC
**** TestResourceByIdNotFound
 #+NAME: test_get_resource_by_id_not_found
#+BEGIN_SRC python
class TestResourceByIdNotFound(TestCase):
    TESTING = True
    ...
    def test_get_resource_by_id_not_found(self):
        print "test_get_resource_by_id_not_found"

        headers = {'Content-Type': 'application/json'}

        response = self.client.get("/map?exp_id=e99847",
                                    headers=headers)
