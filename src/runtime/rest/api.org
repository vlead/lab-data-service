...
#+BEGIN_EXAMPLE
[{"hosting_status": "hosted", "id": 1, "hosted_url": "http://cse14-iiith.vlabs.ac.in"}]
#+END_EXAMPLE	       
...
*** Implementation
#+NAME: update_and_delete_hostinginfo
#+BEGIN_SRC python
@api.route('/hostinginfos', methods=['PUT', 'DELETE'])
def update_and_delete_hostinginfo():
    if request.method == 'PUT':
        if request.is_json:
            data_json = json.dumps(request.get_json())
            data_dict = yaml.safe_load(data_json)
