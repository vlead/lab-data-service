...
#+BEGIN_SRC python
    def test_update_institute(self):
         ...
         self.assertEqual(inst2.get("inst_name"), inst_name1)
...
#+BEGIN_SRC python
    def delete_institute(self, in_id):
        record = Institute.get_by_id(in_id)
        if not record:
            abort(404, 'No Institute with id %s' % (in_id))
        else:
            try:
                record.delete()
                #db.session.delete(record)
...
#+BEGIN_SRC python
    def test_add_saved_labs_to_institute(self):
         ...
         inst_id="IITK"
         institute = Institute(inst_name=inst_name, inst_id=inst_id, labs=[])
         institute.save()
        
         lab_name = "Number Systems"
         lab_id = "EE99777"
