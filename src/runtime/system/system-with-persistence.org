...
class TestUpdateLab(TestCase):
    ...
    def test_update_lab(self):
        ...
        lab1 = System.do("update_lab", lab=lab, lab_name=lab_name2, 
                                  lab_id=lab_id2, overview="overview",
                                  session=session)

        new_lab = lab_cls.get_by_id(1)

        self.assertEqual(new_lab.get("lab_name"), lab_name2)
        self.assertEqual(new_lab.get("lab_id"), lab_id2)
...
* Test UpdateExperiment
  :PROPERTIES:
  :CUSTOM_ID: test_update_experiment
  :END: 
