...
class TestUpdateDeveloper(TestCase):
    ...
    def test_update_developer(self):
        ...
        dev1 = System.do("update_developer", name=dev_name2, email=email_id, session=session)

        new_dev = dev_cls.get_by_id(1)

        self.assertEqual(new_dev.get("name").get("name"), dev_name2.get('name'))
...
class TestGetDeveloper(TestCase):
    ...
    def test_get_developer(self):
        ...
        developer.save()
        developer1 = System.do("get_developer", email=email_id1, session=session)
