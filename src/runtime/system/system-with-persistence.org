...
class TestUpdateAsset(TestCase):
    ...
    def test_update_asset(self):
        ...
        asst1 = System.do("update_asset", asset_type=asset_type2,
                             asset=asset)

        new_asst = asset_cls.get_by_id(1)

        self.assertEqual(new_asst.get("asset_type"), asset_type2)
        self.assertEqual(new_asst.get("path"), path)
...
class TestAddAssetsToLab(TestCase):
    ...
    def test_add_assets_to_lab(self):
        ...
        lab_id = "cse02"
        lab = lab_cls(lab_name=lab_name, lab_id=lab_id)
        key = KEY
        session = session_cls(key=key)
        ...
        asset1 = asset_cls(asset_type="Image")
        asset1.save()

        asset2 = asset_cls(asset_type="Video")
        asset2.save()
