...
class TestAddDeveloper(TestCase):
    ...
    def test_add_developer(self):
        ...
        dev_email = Email(email="dharmar@maths.iitd.ac.in")
        developer = developer_cls(dev_name=dev_name, dev_email=dev_email)
        key = KEY
        session = session_cls(key=key)
        developer = System.do("add_developer", developer=developer, \
                                session=session)
        developer1 = developer_cls.get_by_id(1)
        self.assertEqual(developer1.get("dev_name"), name.get(dev_name))
