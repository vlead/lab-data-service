...
class TestAddExperiment(TestCase):
    ...
    def test_add_experiment_in_system_interface(self):
        ...
        self.assertEqual(experiment.get("sections")[0]['name'], 'Theory')
...
#+begin_src python
    ...
    def update_developer(data_dict):
        ...
        if 'institute' not in data_dict:
            inst=str(developer.get("institute"))
        else:
            inst = System.do("get_institute", inst_id=data_dict['inst_id'])

        if 'discipline' not in data_dict:
            disc=str(developer.get("discipline"))
        else:
            disc = System.do("get_discipline", dis_id=data_dict['dis_id'])
