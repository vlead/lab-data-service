...
#+begin_src python
    ...
    def add_institute(data_dict):
        ...
        labs = data_dict['labs']
        data_dict['labs'] = []
        institute = institute_cls(inst_name=data_dict['inst_name'],
                                  inst_id=data_dict['inst_id'],
                                  labs=data_dict['labs'])
        try:
            current_app.logger.debug("running operation add_institute")
            question = System.do("add_institute", institute=institute, 
                                    session=session)
        ...
        try:
            for lab in labs:
                lab = lab_cls(lab_name=str(lab['lab_name']),
                              lab_id=str(lab['lab_id']))
                lab_list.append(lab)
            current_app.logger.debug("running operation"
                                     "add_labs_to_institutes")
            institute = System.do("add_labs_to_institute",
                                    labs=lab_list,
                                    instid=int(institute.id),
                                    session=session)
            current_app.logger.debug("running operation"
                                     "add_labs_to_institute")
