...
** Filter Columns for a lab
   This function takes a lab and set some of the fields to
   null.  This is required because the to_client() which
   converts an object to a dictionary goes through all the
   attributes of the lab object.  This is not necessary
   since only =assests=, =hosting_info= and =lab_name= are
   required by the front end. 
#+NAME: filter-columns
#+BEGIN_SRC python
    @staticmethod
    def filter_columns(lab):
        current_app.logger.debug("running filter_columns")
        lab.overview = null
        lab.institute = null
        lab.discipline = null
        lab.phase = null
        lab.integration_status = null
        lab.experiments = null
        lab.developers = null
        lab.sections = null
...
#+begin_src python
    ...
    def update_lab(data_dict):
        ...
        if 'sections' not in data_dict['lab']:
            section_obj_list = lab.get("sections")
        else:
            section_list=[]
