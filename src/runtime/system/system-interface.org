...
#        name.save()
#        name=SystemInterface.add_name(data_dict['name'], data_dict['key'])
        email = email_cls(email=data_dict['email'])
#       email.save()
#        email=SystemInterface.add_email(data_dict['email'], data_dict['key'])
        developer = developer_cls(name=str(data_dict['name']), email=(data_dict['email']))
        try:
...
class TestAddDeveloper(TestCase):
    ...
    def test_add_developer_in_system_interface(self):
        ...
        data_dict = {
                    ...
                    'dev_email': 'dharmar@maths.iitd.ac.in'
                   }

        developer = SystemInterface.add_developer(data_dict)
        self.assertEqual(developer.get("name").get("name"),
                             data_dict['dev_name'])
        self.assertEqual(developer.get("email").get("email"), data_dict['dev_email'])
...
class TestGetDevelopers(TestCase):
    ...
    def test_get_developer_in_system_interface(self):
        ...
        self.assertEqual(developer_list[0].get("name"), data_dict['name'])
        self.assertEqual(developer_list[1].get("name"), data_dict1['name'])
...
#+begin_src python
    ...
    def update_institute(data_dict):
        ...
        session = session_cls(key=data_dict['key'])
        inst = inst_cls.get_by_id(data_dict['i_id'])
        ...
        try:
            ...
            question = System.do("update_institute", institute=inst, inst_name=inst_name, 
                                    inst_id=inst_id,
                                    session=session)
            current_app.logger.debug("completed operation update_institute")
            return inst.to_client()
        except (ArityError, TypeError, NotAuthorizedError, StateError) as err:
            current_app.logger.error("Exception = %s" % str(err))
            raise err
...
class TestUpdateInstitute(TestCase):
    ...
    def test_update_institute_name_in_system_interface(self):
        ...
        inst1 = SystemInterface.update_institute(data_dict1)
        
        self.assertEqual(inst1.get("inst_name"), data_dict1['inst_name'])
...
#+begin_src python
    ...
    def update_developer(data_dict):
        ...
        session = session_cls(key=data_dict['key'])
        developer = developer_cls.get_by_id(data_dict['dev_id'])
        ...
        try:
            ...
            developer = System.do("update_developer", name=name, email=email,\
                                             session=session)
...
class TestUpdateDeveloper(TestCase):
    ...
    def test_update_developer_name_in_system_interface(self):
        ...
        self.assertEqual(developer1.get("name"), data_dict1['name'])
 #+END_SRC
...
#+begin_src python
    @staticmethod
    def delete_institute(i_id):
        session_cls = System.delegate.entities['session']

        session = session_cls(key=KEY)
        try:
            current_app.logger.debug("running operation delete_institute")
            inst = System.do("delete_institute", i_id=int(i_id),
                                    session=session)
...
class TestDeleteInstitute(TestCase):
    ...
    def test_delete_institute_in_system_interface(self):
        ...
        inst = SystemInterface.add_institute(data_dict)
        inst1 = SystemInterface.add_institute(data_dict1)
        i_id = SystemInterface.delete_institute(1)
        
        self.assertEqual(i_id, 1)
...
class TestDeleteName(TestCase):
    ...
    def test_delete_name_in_system_interface(self):
        ...
        name_id = SystemInterface.delete_name(data_dict1)
...
class TestDeleteEmail(TestCase):
    ...
    def test_delete_email_in_system_interface(self):
        ...
        email_id = SystemInterface.delete_email(data_dict1)
...
#+begin_src python
    @staticmethod
    def delete_developer(dev_id, key):
        session_cls = System.delegate.entities['session']

        session = session_cls(key=KEY)
        try:
            current_app.logger.debug("running operation delete_developer")
            developer = System.do("delete_developer", dev_id=int(dev_id),
                                    session=session)
...
class TestDeleteDeveloper(TestCase):
    ...
    def test_delete_developer_in_system_interface(self):
        ...
        data_dict1 = {
                    'key' : KEY,
                    'dev_name' : 'Prof. Raja',
                    'dev_email': 'xyz@gmail.com'
                   }

        developer = SystemInterface.add_developer(data_dict)
        developer1 = SystemInterface.add_developer(data_dict1)
        dev_id = SystemInterface.delete_developer(1)
...
<<test_class_get_name>>
<<test_class_get_email>>
<<test_class_get_developer>>
