...
class TestDeleteHostingInfo(TestCase):
    ...
    def test_delete_hostinginfo_in_system_interface(self):
        ...
        hosted_url = SystemInterface.delete_hostinginfo("http://iitkgp.vlab.co.in", KEY)
        
        self.assertEqual(hosted_url, "http://iitkgp.vlab.co.in")
...
#+begin_src python
    @staticmethod
    def delete_developer(email_id, key):
        session_cls = System.delegate.entities['session']
        email_cls = System.delegate.entities['email']

        session = session_cls(key=str(key))
        email = email_cls(email=email_id)
...
#+begin_src python
    ...
    def delete_integrationstatus(integration_level, key):
        ...
        try:
            ...
            integrationstatus_id = System.do("delete_integrationstatus", integration_level=int(integration_level),
                                      session=session)
...
#+begin_src python       
    ...
    def update_asset(data_dict):
        ...
        if 'asset_path' not in data_dict:
            asset_path=str(asset.get("asset_path"))
        else:
            asset_path=data_dict['asset_path']
...
class TestUpdateAsset(TestCase):
    ...
    def test_update_asset_name_in_system_interface(self):
        ...
        self.assertEqual(asset1.get("asset_type"), data_dict1['])
...
class TestGetAssets(TestCase):
    ...
    def test_get_assets_in_system_interface(self):
        ...
        data_dict = {'asset_type': 'Image',
                    'path': 'vlabs.ac.in/static/images/logo.png',
                    'key' : KEY
                    }

        data_dict1 = {'asset_type': 'Video',
                    'path': 'vlabs.ac.in/static/images/icon.png',
                    'key' : KEY
                    }

        asset1 = SystemInterface.add_asset(data_dict)
        asset2 = SystemInterface.add_asset(data_dict1)

        asset_list = SystemInterface.get_assets()
        
        self.assertEqual(asset_list[0].get("asset_type"), data_dict['asset_type'])
        self.assertEqual(asset_list[1].get("asset_type"), data_dict['asset_type']
        self.assertEqual(asset_list[0].get("path"), 
                             data_dict['path'])
        self.assertEqual(asset_list[1].get("path"), 
                             data_dict['path'])
