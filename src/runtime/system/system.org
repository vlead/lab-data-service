...
class UpdateDeveloper():
    ...
    def action(args):
        email = args["email"]
        developer = System.delegate.get_developer(email=email)
        if not "email" in args:
           name=developer.get("name")
        else:
           name=args["name"]

        developer = System.delegate.update_developer(developer, name, email)
        return developer
...
class DeleteDeveloper():
    ...
    def state_check(args):
        email = args["email"]
        if not System.delegate.developer_exists(email):
            raise StateError("developer with email_id: %s does not exists in System"
                                % email.to_client())
