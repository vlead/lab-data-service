...
# python :tangle setup.py :eval no
#+BEGIN_SRC python
...
# python :tangle dump_json_to_lds.py :eval no 
...
apt-get install -y python-dev libmysqld-dev python-setuptools 
if [[ $? -ne 0 ]]; then
...
* Configuring the application and its deployment
  The following program configures the application, and calls the database
...
# 2. Run the database setup scripts to setup the database
...
export PYTHONPATH="/usr/share/nginx/html"
setup_db
exit 0;
...
#+BEGIN_SRC python :tangle lds.conf :eval no

description "Gunicorn application server running lab-data-service"
...
exec gunicorn --workers 3 --bind unix:lds.sock -m 007 wsgi

#+END_SRC

** nginx socket configuration

#+BEGIN_SRC python :tangle lds :eval no
...
server {
    ...
    location / {
        ...
        proxy_pass http://unix:/usr/share/nginx/html/deployment/lds.sock;
    }
}
...
git clone https://github.com/vlead/lab-data-service
cd lab-data-service
git checkout develop
make
rsync -avz --progress build/code/ /var/www/lds
cd /var/www/lds
virtualenv venv
...
#+END_EXAMPLE
** Create a WSGI Application =emacs /var/www/lds/wsgi.py=
   
#+BEGIN_EXAMPLE
import sys
sys.path.insert(0, "/var/www/lds")
...
#+END_EXAMPLE
** Configure a uWSGI Config File =emacs /var/www/lds/lds.ini=
#+BEGIN_EXAMPLE
[uwsgi]
...
die-on-term = true
#+END_EXAMPLE
** Create an Upstart File to Manage the App =emacs /etc/init/lds.conf=
#+BEGIN_EXAMPLE
description "uWSGI instance to serve lds"
...
** Configure Nginx to Proxy to uWSGI =emacs /etc/nginx/sites-available/lds=
#+BEGIN_EXAMPLE
server {
    listen 80;
    server_name localhost;
    ...
    location / {
        ...
        uwsgi_pass      unix:/var/www/lds/lds.sock;
    }
}
...
#+BEGIN_EXAMPLE
ln -s /etc/nginx/sites-available/lds /etc/nginx/sites-enabled
#+END_EXAMPLE
